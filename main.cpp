...
template <typename T>
bool isEmpry(BinarySearchTree<T> &bts) {
...
// Ham Ho Tro
void print(MonHoc &monhoc) {
	cout << monhoc.maMH << "\t" << monhoc.tenMH << "\t"
		<< monhoc.STCLY << "\t" << monhoc.STCTH << endl;
}
...
void inputMonHoc(MonHoc &mh) {
	do {
		...
		gets(mh.maMH);
		if(strlen(mh.maMH) <= 10) break;
		else cout << "Ma Mon Hoc Toi Da 10 Ky Tu\n";
	} while (true);
	...
	do {
		...
		gets(mh.tenMH);
		if(strlen(mh.tenMH) <= 50) break;
		else cout << "Ten Mon Hoc Toi Da 50 Ky Tu\n";
	} while (true);
	...
	do {
		...
		cin >> mh.STCLY;
		if(0 < mh.STCLY && mh.STCLY <= 3) break;
		else cout << "STCLY (0-3)\n";
	} while (true);
	...
	do {
		...
		cin >> mh.STCTH;
		if(0 < mh.STCTH && mh.STCTH <= 3) break;
		else cout << "STCTH (0-3)\n";
	} while (true);
} 
